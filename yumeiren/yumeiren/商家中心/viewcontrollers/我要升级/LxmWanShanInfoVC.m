...
@property (nonatomic, strong) LxmJieDanPublishTextFieldCell *shopNameCell;//姓名
...
@property (nonatomic, strong) LxmJieDanPublishTextFieldCell *addressCell;//详细地址输入
...
- (LxmJieDanPublishTextFieldCell *)shopNameCell {
    if (!_shopNameCell) {
        ...
        _shopNameCell.titleLabel.text = @"姓名";
...
- (LxmJieDanPublishTextFieldCell *)addressCell {
      if (!_addressCell) {
          _addressCell = [[LxmJieDanPublishTextFieldCell alloc] init];
          _addressCell.titleLabel.text = @"详细地址";
          _addressCell.textField.placeholder = @"请输入详细地址";
          if (LxmTool.ShareTool.userModel.shopInfo.addressDetail.isValid) {
              _addressCell.textField.textColor = CharacterDarkColor;
              _addressCell.textField.text = LxmTool.ShareTool.userModel.shopInfo.addressDetail;
              _addressCell.textField.userInteractionEnabled = NO;
          }
      }
      return _addressCell;
}
...
//    }];
    [self.quGouwuButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.addressCell.mas_bottom).offset(20);
...
- (void)gotoGouWuButtonClick {
    ...
    if (self.shopNameCell.textField.text.length > 20) {
        [SVProgressHUD showErrorWithStatus:@"姓名在1~20个字符!"];
        return;
    }
