...
@property(nonatomic,strong)UIButton *xieYiBt,*yinSiBt;
...
- (UIButton *)xieYiBt {
    if (!_xieYiBt) {
        _xieYiBt = [[UIButton alloc] init];
        [_xieYiBt setTitle:@"《用户协议》" forState:UIControlStateNormal];
        [_xieYiBt setTitleColor:MainColor forState:UIControlStateNormal];
        _xieYiBt.titleLabel.font = [UIFont systemFontOfSize:14];
        [_xieYiBt addTarget:self action:@selector(xieyiAction:) forControlEvents:UIControlEventTouchUpInside];
        _xieYiBt.tag = 100;
    }
    return _xieYiBt;
}
- (UIButton *)yinSiBt {
    if (!_yinSiBt) {
        _yinSiBt = [[UIButton alloc] init];
        [_yinSiBt setTitle:@"《隐私政策》" forState:UIControlStateNormal];
        [_yinSiBt setTitleColor:MainColor forState:UIControlStateNormal];
        _yinSiBt.titleLabel.font = [UIFont systemFontOfSize:14];
        [_yinSiBt addTarget:self action:@selector(xieyiAction:) forControlEvents:UIControlEventTouchUpInside];
        _yinSiBt.tag = 101;
    }
    return _yinSiBt;
}
...
//    self.headerView.backgroundColor = [UIColor redColor];
    ...
    [self.headerView addSubview:self.xieYiBt];
    [self.headerView addSubview:self.yinSiBt];
...
- (void)setConstrains {
    ...
    [self.xieYiBt mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.loginButton.mas_bottom).offset(60);
        make.centerX.equalTo(self.headerView).offset(-50);
        make.height.equalTo(@30);
    }];
    
    
    [self.yinSiBt mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.loginButton.mas_bottom).offset(60);
        make.centerX.equalTo(self.headerView).offset(50);
        make.height.equalTo(@30);
    }];
    ...
    [self.textLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.xieYiBt.mas_bottom).offset(60);
...
//协议部分
- (void)xieyiAction:(UIButton *)button {
    if (button.tag == 100) {
        LxmWebViewController * web = [[LxmWebViewController alloc] init];
        NSString *bundleStr = [[NSBundle mainBundle] pathForResource:@"ruleDetailProvince" ofType:@"html"];
        ...
        LxmWebViewController * web = [[LxmWebViewController alloc] init];
        NSString *bundleStr = [[NSBundle mainBundle] pathForResource:@"submitRule" ofType:@"html"];
        NSURL * url =  [NSURL fileURLWithPath:bundleStr];
        web.loadUrl = url;
        web.navigationItem.title = @"隐私协议";
        [self.navigationController pushViewController:web animated:YES];
    }
}
